%% SRK Package
function [Z,phi,H,V,S]= srk(x,T,P,phase)
% In: compositon x (vector), Temperature T [K], pressure p [N/m2], phase ['liquid' or 'vapor']
% OUT: Compressibility Z, Fugacity coeff.(vector) phi [-], enthalpy H [J/mol], Molar Volume V [m3/mol]
...
% Use real roots only
Zall=Zall(imag(Zall)==0);

% Select right root depending on phase
if     phase==liquid  Z=min(Zall);
elseif phase==vapor   Z=max(Zall);
else   write('error in specifying phase')
end
